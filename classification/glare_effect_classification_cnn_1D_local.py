...
def load_data_splits(steps, base_path= cwd + '/../classification_data/features/', splits=20):
...
def create_and_train(X_train, y_train, X_test, y_test, run ,split, file_dir, sd, n_epochs=n_epochs):
    ...
    json.dump(history.history, open(file_dir + '/split_histories_sd%sx/split%s/history_run%s' %(sd, split, run), 'w'))
...
def load_histories(file_dir, sd):
    ...
    for split in range(splits):
        ...
        for run in range(n_runs):
            history = json.load(open(file_dir + '/split_histories_sd%sx/split%s/history_run%s' %(sd, split, run), 'r'))
...
def split_training(split):
    ...
    X_train = real_data_splits_train[split][0]
    y_train = real_data_splits_train[split][1]
    X_test = real_data_splits_test[split][0]
    y_test = real_data_splits_test[split][1]
    
    # Adding simulated data. 
    X_train, y_train = add_simulated_data(X_train, y_train, simulated_data_splits_train[split], n_added_simulations_per_participant)
...
if __name__ == "__main__":
    ...
    global real_data_splits_train, real_data_splits_test, simulated_data_splits_train
    real_data_splits_train, real_data_splits_test, simulated_data_splits_train = \
        load_data_splits(steps=20)

    now = datetime.now()
    time = now.strftime('%Y-%m-%d %H:%M:%S.%f')[:-2]
    global file_dir
    file_dir = cwd + '/1d_cnn_%s' %time
    ...
    for n_added_simulations_per_participant in numbers_of_simulation:
        ...
        plt.savefig(file_dir + '/sd%sx_acc.png' %n_added_simulations_per_participant)
        plt.clf()
        ...
        plt.savefig(file_dir + '/sd%sx_loss.png' %n_added_simulations_per_participant)
        plt.clf()