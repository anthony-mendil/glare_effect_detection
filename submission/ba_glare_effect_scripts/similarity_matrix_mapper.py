...
# This script is used to collect the statically mapped as well as the 
# dynamically mapped logs with static similarity assignments 
# and create new logs with dynamically mapped similarity assignments.
...
def map_similarity_matrix(original_log, mapped_cards_original_sim_matrix_log):
    ...
    :param mapped_cards_original_sim_matrix_log: It has dynamic card coding but its similarity matrix entries 
    use original static coding (non mapped)!
    ...
    # Replacing old similarity values in the log with the new ones. 
    new_log = []
    ...
    # Resulting data frame of mapped motives with corresponding mapped similarity matrix to be prepared for all games.
    mapped_motives_mapped_sim_matrix = pd.DataFrame(columns=mapped_col_names)
    # Forcing ts columns to int64 data type (to avoid automatic non-precise float data type).
    for ts in cards_ts:
        mapped_motives_mapped_sim_matrix = mapped_motives_mapped_sim_matrix.astype({ts: int})

    # Mapping the similarity matrix.
    # The mapper.
    motives_mapper = {}
    ...
    for assignment in similarities_assignments:
        ...
        # Adding the mapped assignment. 
        mapped_motives_mapped_sim_matrix.at[0, assignment] = mapped_assignment

    # Setting the mapped cards, ts , statistics and labels of this game: game_index.
...
if __name__ == "__main__":
    ...
    # Saving the logs with dynamically mapped similarity matrix entries. 
